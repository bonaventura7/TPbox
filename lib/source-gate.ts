import { extractDomain, isAllowedHost, canonicalUrl } from './whitelist';
...
export async function runSourceGate(input:GateInput,ctx:GateContext):Promise<GateDetail>{
 ...
 if(!domain||!isAllowedHost(domain))return{result:'FAIL_DOMAIN',checks,reason:`dominio non ammesso: ${domain||'(vuoto)'}`}; checks.domain=`ok (${domain})`;
