...
const ctx = {
  async checkHttp(url: string) {
    return (await http(url)).status;
  },
  async checkPdf(url: string) {
  ...
  async isDuplicate(url: string) {
    const { data } = await serviceClient()
      .from("news_items")
      .select("id")
      .eq("source_url", canonicalUrl(url))
      .eq("status", "PUBLISHED")
      .limit(1);
...
Deno.serve(async (req: Request) => {
  const auth = await authorizeCaller(
    req,
    authClient,
    Deno.env.get("NEWS_PUBLISH_CALLER_USER_ID"),
    "news-publish",
  );
  ...
  if (!AUTO_PUBLISH)
    return json({
      ...
      blocked: 0,
      kill_switch: true,
      note: "AUTO_PUBLISH_ENABLED=false — nessun articolo pubblicato",
    });
  ...
  for (const it of items ?? []) {
    ...
    if (it.status !== "DRAFT" || !gateOk) {
      ...
      details.push({
        id: it.id,
        result: "BLOCKED",
        gate: "FAIL_GATE_RESULT",
        reason: "gate_result.ok !== true",
      });
      continue;
    }
    ...
    if (contentLength < MIN_PUBLISHABLE_CONTENT_CHARS) {
      await supabase
        ...
        .update({
          ...
          gate_result: {
            ok: false,
            reasons: [
              `articolo troppo breve: ${contentLength} caratteri (minimo ${MIN_PUBLISHABLE_CONTENT_CHARS})`,
            ],
          },
        })
        .eq("id", it.id)
      ...
      details.push({
        ...
        result: "BLOCKED",
        gate: "FAIL_TOO_SHORT",
        reason: `content_markdown ${contentLength} < ${MIN_PUBLISHABLE_CONTENT_CHARS}`,
      });
      continue;
    }
    const gate = await runSourceGate(
      {
        sourceUrl: it.source_url,
        pdfUrl: it.pdf_url ?? null,
        bodyText: it.content_markdown ?? it.summary ?? "",
        references: Array.isArray(it.normative_references) ? it.normative_references : [],
    ...
    await supabase
      ...
      .insert({
        ...
        details: { ...gate.checks, reason: gate.reason ?? null },
      });
    if (isPass(gate)) {
      ...
      if (updateError) {
        blocked++;
        details.push({ id: it.id, result: "UPDATE_FAILED" });
      } else {
        published++;
        details.push({ id: it.id, result: "PUBLISHED" });
      }
    } else {
      await supabase.from("news_items").update({ status: "DRAFT" }).eq("id", it.id);
      blocked++;
      details.push({ id: it.id, result: "BLOCKED", gate: gate.result, reason: gate.reason });
    }
  }
