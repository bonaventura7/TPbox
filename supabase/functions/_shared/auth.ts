...
export type AuthResult =
  { ok: true; userId: string; correlationId: string } | { ok: false; response: Response };
...
export async function authorizeCaller(
  ...
  if (!authorization.startsWith("Bearer ")) {
    return {
      ok: false,
      response: jsonResponse({ ok: false, error: "missing_authorization" }, 401, cid),
    };
  }
  ...
  if (!expectedCallerId) {
    console.error(
      JSON.stringify({ event: `${serviceName}.auth_config_error`, correlation_id: cid }),
    );
    return {
      ok: false,
      response: jsonResponse({ ok: false, error: "authorization_not_configured" }, 500, cid),
    };
  }
  ...
  if (userError || !user) {
    ...
    return {
      ok: false,
      response: jsonResponse({ ok: false, error: "invalid_identity" }, 401, cid),
    };
  }
  ...
  if (!decision.ok) {
    console.warn(
      JSON.stringify({
        event: `${serviceName}.auth_forbidden`,
        correlation_id: cid,
        user_id: user.id,
      }),
    );
    return {
      ok: false,
      response: jsonResponse({ ok: false, error: decision.error }, decision.status, cid),
    };
  }
