...
export type DraftValidation = { ok: true; value: ValidDraft } | { ok: false; reason: string };

export function validateDraft(value: unknown): DraftValidation {
  if (!value || typeof value !== "object")
    return { ok: false, reason: "risposta LLM non strutturata" };
  ...
  if (title.length < 25 || title.length > 180)
    return { ok: false, reason: `titolo non valido (${title.length} caratteri)` };
  if (summary.length < 100 || summary.length > 500)
    return { ok: false, reason: `sommario non valido (${summary.length} caratteri)` };
  if (content.length < 1200)
    return { ok: false, reason: `articolo troppo breve (${content.length} caratteri)` };
  if (!DRAFT_CATEGORIES.includes(category))
    return { ok: false, reason: `categoria non valida: ${category}` };

  if (
    !Array.isArray(draft.normative_references) ||
    draft.normative_references.some((item) => typeof item !== "string")
  ) {
