...
async function generate(sourceText: string, sourceDomain: string) {
  ...
  const { value } = await generateJson(
    [{ role: 'system', content: system }, { role: 'user', content: sourceText }],
    { OPENROUTER_API_KEY: Deno.env.get('OPENROUTER_API_KEY'), LLM_MODEL: Deno.env.get('LLM_MODEL'), LLM_MODEL_FALLBACK: Deno.env.get('LLM_MODEL_FALLBACK'), LLM_ENDPOINT: Deno.env.get('LLM_ENDPOINT'), LLM_TIMEOUT_MS: Deno.env.get('LLM_TIMEOUT_MS'), LLM_MAX_RETRIES: Deno.env.get('LLM_MAX_RETRIES') },
  );
...
Deno.serve(async (req: Request) => {
  ...
  try { const body = await req.json().catch(() => ({})); batchSize = clamp(Number(body.batch_size ?? DEFAULT_BATCH_SIZE), 1, MAX_BATCH_SIZE); timeBudgetMs = clamp(Number(body.time_budget_seconds ?? 110) * 1000, MIN_TIME_BUDGET_MS, DEFAULT_TIME_BUDGET_MS); } catch {}
  ...
  if (error) return jsonResponse({ ok: false, error: 'persistence_failed' }, 500, cid);
  ...
  const created: string[] = []; let blocked = 0; let remaining = 0;
  for (const d of discoveries ?? []) {
    const elapsedMs = Date.now() - startedAt;
    if (elapsedMs + SAFETY_MARGIN_MS > timeBudgetMs) { remaining += 1; break; }
    try {
      const sourceText = await fetchText(d.source_url); const domain = extractDomain(d.source_url); const source = findSource(domain); if (!source) throw new Error(`dominio non whitelistato: ${domain}`);
      const draft = await generate(sourceText, domain); if (!CATEGORIES.includes(String(draft.category))) throw new Error(`categoria non valida: ${String(draft.category)}`);
      const category = CATEGORY_DB[String(draft.category)]; const refs = Array.isArray(draft.normative_references) ? draft.normative_references.filter((x: unknown) => typeof x === 'string').map((x: string) => x.trim()) : [];
      const unknown = refs.filter((r: string) => !known.has(r.toLowerCase())); if (unknown.length) throw new Error(`riferimenti normativi non presenti nel catalogo: ${unknown.join('; ')}`);
      const { data: dup } = await supabase.from('news_items').select('id').eq('source_url', d.source_url).maybeSingle(); if (dup) { await supabase.from('news_discovery').update({ status: 'GENERATED', gate_result: 'PASS_DUPLICATE' }).eq('id', d.id); continue; }
      const { error: insertError } = await supabase.from('news_items').insert({ slug: slugify(String(draft.title)), title: String(draft.title), summary: String(draft.summary ?? '').slice(0, 500), content_markdown: String(draft.content_markdown ?? ''), category, country: String(draft.country ?? 'INT'), source_name: source.name, source_url: d.source_url, pdf_url: d.pdf_url ?? null, normative_references: refs, status: 'DRAFT', fetched_at: new Date().toISOString() }); if (insertError) throw insertError;
      await supabase.from('news_discovery').update({ status: 'GENERATED', gate_result: 'PASS' }).eq('id', d.id); created.push(String(draft.title));
    } catch (e) { blocked += 1; const safeError = redactSecret(e instanceof Error ? e.message : String(e)); console.error(JSON.stringify({ event: 'news-generate.item_failed', correlation_id: cid, discovery_id: d.id, error: safeError })); await supabase.from('news_discovery').update({ status: 'BLOCKED', gate_result: 'FAIL_UNKNOWN', error: safeError }).eq('id', d.id); }
  }
  const elapsedMs = Date.now() - startedAt; const partial = remaining > 0;
