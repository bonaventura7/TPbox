...
Deno.serve(async (req: Request) => {
  const auth = await authorizeCaller(
    req,
    authClient,
    Deno.env.get("NEWS_SCOUT_CALLER_USER_ID"),
    "news-scout",
  );
  ...
  for (const item of items) {
    ...
    if (!primary) {
      blocked++;
      results.push({ title: item.title, status: "BLOCKED", gate_result: "FAIL_DOMAIN" });
      continue;
    }
