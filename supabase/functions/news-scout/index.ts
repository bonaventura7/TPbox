...
function extractPrimaryUrl(raw:string, fallback:string): string | null {
  ...
  for (const url of [fallback, ...candidates]) {
    ...
    if (findSource(d)) return canonicalUrl(url);
  }
  return null;
}
...
async function fetchStatus(url:string):Promise<number>{
  ...
  return (await fetch(url,{headers:h,signal:AbortSignal.timeout(20000)})).status;
}
function json(body:unknown,status=200){return new Response(JSON.stringify(body),{status,headers:{'content-type':'application/json'}})}

Deno.serve(async () => {
  let xml:string;
  try { const r=await fetch(FEED,{headers:{'user-agent':UA,accept:'application/rss+xml, application/atom+xml, text/xml'},signal:AbortSignal.timeout(30000)}); if(!r.ok) throw new Error(`feed HTTP ${r.status}`); xml=await r.text(); }
  catch(e){ return json({ok:false,error:String(e)},502); }

  const items=parseFeed(xml); const results=[];
  for(const item of items){
    ...
    const domain=extractDomain(primary); const source=findSource(domain)!;
    ...
    const status=http>=200&&http<300?'SCOUTED':'BLOCKED';
    const gate_result=status==='SCOUTED'?'PASS':'FAIL_HTTP';
  ...
  return json({ok:true,scouted:items.length,results});
});
