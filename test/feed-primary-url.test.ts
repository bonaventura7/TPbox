...
import {
  extractPrimaryUrl,
  isSpecificPrimaryUrl,
  parseFeed,
} from "../supabase/functions/_shared/feed-primary-url";
...
describe("RegFollower primary URL extraction", () => {
  ...
  it("decodes query entities in official href values", () => {
    const raw =
      '<a href="https://www.canada.ca/en/finance/news.html?id=12&amp;lang=en">release</a>';
    expect(extractPrimaryUrl(raw, "")).toBe(
      "https://www.canada.ca/en/finance/news.html?id=12&lang=en",
    );
  });
  ...
  it("rejects institutional homepages as non-news URLs", () => {
    ...
    expect(
      extractPrimaryUrl("Visit https://www.sii.cl/", "https://regfollower.com/item"),
    ).toBeNull();
  });
