...
const CBE = "0442824497"; // BEAULIEU International Group — caso di verifica
const KEY = "test-primary-key";

/** Risposta /references ridotta: due depositi, vince il più recente. */
const REFERENCES = {
  legalEntity: CBE,
  references: [
...
describe("Belgio — CBE e chiamata /references", () => {
  ...
  it("chiama /references con chiave e X-Request-Id e serve l'ultimo deposito", async () => {
    ...
    // Eccezione-contratto documentata: URL già proxato, con accept=pdf.
    ...
    // La chiave non lascia mai il server: non è nell'URL servito al browser.
...
describe("Belgio — il proxy inietta la chiave solo lato server", () => {
  ...
  it("con chiave configurata la inoltra al gateway e serve il PDF", async () => {
    ...
    // La chiave non lascia mai il server: né corpo né header la espongono.
