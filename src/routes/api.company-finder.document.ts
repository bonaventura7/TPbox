...
const SUPABASE_RELAY = "https://igtthymjeujkgfpmgoqj.supabase.co/functions/v1/poland-financial-relay-v2";
const SUPABASE_PUBLISHABLE_KEY = "sb_publishable_-SISHSkYmXMacDgCbvlaXQ_P0bAKx1U";
...
export const Route = createFileRoute("/api/company-finder/document")({
  server: { handlers: { GET: async ({ request }: { request: Request }) => {
    ...
    if (country === "PL") {
      ...
      const relay = await fetch(SUPABASE_RELAY, {
        method: "POST",
        headers: { "Authorization": `Bearer ${SUPABASE_PUBLISHABLE_KEY}`, "apikey": SUPABASE_PUBLISHABLE_KEY, "Content-Type": "application/json", Accept: "application/pdf,*/*" },
        body: JSON.stringify({ krs, year }),
        cache: "no-store",
      });
      if (relay.ok) {
        const bytes = new Uint8Array(await relay.arrayBuffer());
        return new Response(bytes, { status: 200, headers: { "Content-Type": relay.headers.get("content-type") || "application/pdf", "Content-Disposition": `${download ? "attachment" : "inline"}; filename="bilancio-${krs}-${year}.pdf"`, "Cache-Control": "no-store", "X-Content-Type-Options": "nosniff" } });
      }
