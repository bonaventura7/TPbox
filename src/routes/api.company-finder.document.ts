    ...
    handlers: {
      GET: async ({ request }: { request: Request }) => {
        ...
        if (country === "PL") {
          ...
          const { fetchPolishAnnualReport } =
            await import("@/lib/company-finder/sources/bilanci/poland-rdf");
          ...
          if (!result.ok) {
            const { polishOfficialFallbackResponse } =
              await import("@/lib/company-finder/pl-official-fallback");
            return polishOfficialFallbackResponse(krs);
          }
        ...
        if (country === "EE") {
          ...
          const { fetchEeFileDocument, resolveEeFiling } =
            await import("@/lib/company-finder/sources/bilanci/ariregister-ee");
          const filing = await resolveEeFiling(code, year);
          if (!filing.ok || !filing.fileId)
            return errorResponse(filing.error ?? `bilancio ${year} non disponibile`, 502);
          const doc = await fetchEeFileDocument(code, filing.fileId);
          if (!doc.ok || !doc.bytes)
            return errorResponse(doc.error ?? `bilancio ${year} non disponibile`, 502);
          ...
          return new Response(doc.bytes, {
            ...
            headers: {
              "Content-Type":
                doc.contentType ?? (isPdf ? "application/pdf" : "text/html; charset=utf-8"),
        ...
        if (country === "NO") {
          ...
          const { fetchBrregAnnualReportDocument } =
            await import("@/lib/company-finder/sources/bilanci/brreg-no");
          const doc = await fetchBrregAnnualReportDocument(orgnr, year, 30000);
          if (!doc.ok || !doc.bytes)
            return errorResponse(doc.error ?? `bilancio ${year} non disponibile`, 502, {
              fallback: "official-registry",
            });
          return new Response(doc.bytes, {
            status: 200,
            headers: {
