...
export function Prose({ children }: { children: ReactNode }) {
  return <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">{children}</div>;
}
    ...
    <ul className="grid gap-4 md:grid-cols-2">
      {items.map((item) => (
        <li key={item.title} className="border border-border bg-card p-5">
          ...
          <p className="mt-3 text-xs tracking-wide text-muted-foreground uppercase">{item.meta}</p>
        </li>
      ))}
    </ul>
  );
