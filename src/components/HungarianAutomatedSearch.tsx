  ...
  if (searching) {
    return (
      <div className="text-center py-12">
        <svg
          className="animate-spin mx-auto h-16 w-16 text-blue-600"
          fill="none"
          viewBox="0 0 24 24"
        >
          <circle
      ...
      <div className="space-y-4">
        ...
        {uploadedDocumentId ? (
          <DocumentViewer
            ...
            title={`Bilancio - ${result.companyName}`}
          />
        ) : (
          <HungarianDocumentUpload
            companyId={`HU-${result.companyId || "unknown"}`}
            onDocumentUploaded={setUploadedDocumentId}
          />
        )}
      </div>
    );
  ...
  if (result?.error) {
    return (
      <div className="space-y-4">
        ...
        <HungarianDocumentUpload
          companyId={`HU-${companyName.replace(/[^a-z0-9]/gi, "")}`}
          onDocumentUploaded={setUploadedDocumentId}
        />
      </div>
    );
