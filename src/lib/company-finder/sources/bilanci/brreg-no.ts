...
/**
 * Il servizio ha avuto rappresentazioni JSON differenti tra versioni: la
 * funzione accetta sia array di anni sia wrapper oggettuali con chiavi comuni.
 */
export function parseBrregAnnualYears(payload: unknown): number[] {
  ...
  if (payload && typeof payload === "object") {
    ...
    for (const key of ["aar", "år", "years", "availableYears", "regnskapsaar", "regnskapsår"]) {
...
export async function fetchBrregAnnualReports(
  ...
  try {
    const yearUrl = `${BASE}/${code}/aar`;
    const response = await fetchWithTimeout(yearUrl, timeoutMs);
