...
async function searchOfficialUr(companyName: string, timeoutMs: number): Promise<UrResult> {
  ...
  try {
    ...
    if (!publications.length) return { ok: true, data: { available: false, years: [], source: "Unternehmensregister (DE)", note: "Nessuna pubblicazione contabile trovata." } };
    ...
    const chosen = sameCompany.find((p) => /abschl|bilanz|finanzbericht|annual/i.test(p.title ?? "")) ?? sameCompany[0] ?? publications[0]; if (!chosen) return { ok: true, data: { available: false, years: [], source: "Unternehmensregister (DE)", note: "Nessuna pubblicazione contabile trovata." } };
    const payload = chosen.payload ?? chosen.encryptedPayload; if (!payload) return { ok: true, data: { available: false, years: [], source: "Unternehmensregister (DE)", note: "Pubblicazione trovata ma documento non accessibile automaticamente." } };
    const param = chosen.payload ? "payload" : "encryptedPayload"; const publicationUrl = `${UR_BASE}/de/veroeffentlichung?${param}=${encodeURIComponent(payload)}`; const proxy = `/api/company-finder/document?url=${encodeURIComponent(publicationUrl)}`; const yearMatch = (chosen.sourceDate ?? chosen.title ?? "").match(/20\d{2}/);
    return { ok: true, data: { available: true, years: [], source: `Unternehmensregister (DE) — ${chosen.title ?? "Jahresabschluss"}`, documentUrl: proxy, documentTitle: [chosen.title, chosen.companyNameAtTimeOfPublication, chosen.sourceDate].filter(Boolean).join(" · "), availability: "DOCUMENT_DOWNLOADABLE", documents: [{ id: `de-${payload.slice(0, 20)}`, year: yearMatch ? Number(yearMatch[0]) : undefined, kind: "ANNUAL_REPORT", format: chosen.hasPdf === false ? "html" : "pdf", availability: "DOCUMENT_DOWNLOADABLE", title: chosen.title, downloadUrl: `${proxy}&download=1` }], note: "Documento ufficiale individuato sul registro tedesco e servito dal backend TPBox tramite proxy same-origin." } };
