...
function rscBlob(html: string): string {
  ...
  for (const chunk of chunks) {
    ...
    if (!match) continue;
    try { out.push(JSON.parse(`"${match[1]}"`)); } catch { /* ignore malformed */ }
  }
  return out.join("\n");
}
...
function extractObjects(blob: string, marker: string): JsonObject[] {
  ...
  while (true) {
    ...
    if (blob[start] !== "{") { cursor = start; continue; }
    ...
    if (end < 0) break;
    try { out.push(JSON.parse(blob.slice(start, end + 1)) as JsonObject); } catch { /* ignore */ }
    cursor = end + 1;
  }
  return out;
}
...
async function searchOfficialUr(companyName: string, timeoutMs: number): Promise<UrResult> {
  ...
  try {
    ...
    const tokenRes = await fetch(`${UR_BASE}/api/search-token`, {
      headers: { "User-Agent": UA, Accept: "application/json", ...(cookie ? { Cookie: cookie } : {}), Referer: `${UR_BASE}/de/suche` },
      signal: ctrl.signal,
      cache: "no-store",
    });
    if (!tokenRes.ok) return { ok: false, error: `UR search-token HTTP ${tokenRes.status}` };
    const tokenBody = (await tokenRes.json()) as { token?: string };
    if (!tokenBody.token) return { ok: false, error: "UR: token assente" };
    ...
    if (!companies.length && !publications.length) {
      return { ok: true, data: { available: false, years: [], source: "Unternehmensregister (DE)", note: "Nessuna pubblicazione contabile trovata." } };
    }
    ...
    const sameCompany = publications.filter((p) => {
    ...
    return {
      ...
      data: {
        available: true,
        years: [],
        source: `Unternehmensregister (DE) — ${chosen.title ?? "Jahresabschluss"}`,
        documentUrl: `/api/company-finder/document?url=${encodeURIComponent(publicationUrl)}`,
        documentTitle: [chosen.title, chosen.companyNameAtTimeOfPublication, chosen.sourceDate].filter(Boolean).join(" · "),
        note: "Il documento è individuato sul registro ufficiale; il recupero server-side può essere limitato dalla verifica di sicurezza del portale.",
      },
    };
  } catch (e) {
    const err = e as { name?: string; message?: string; cause?: { code?: string } } | undefined;
    const code = err?.cause?.code;
    return { ok: false, error: `Unternehmensregister: ${err?.name === "AbortError" ? "timeout" : code === "ENOTFOUND" ? "DNS non risolto" : err?.message ?? "errore"}` };
  } finally {
    clearTimeout(timer);
  }
}
...
export async function searchUrAccounting(companyName: string, timeoutMs = 30000): Promise<UrResult> {
  const key = env().OPENREGISTER_API_KEY;
  if (key) {
