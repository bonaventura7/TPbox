...
function appendSetCookies(cookie: string, headers: Headers): string {
  ...
  for (const part of cookie.split("; ")) { const eq = part.indexOf("="); if (eq > 0) jar.set(part.slice(0, eq), part); }
  for (const raw of values) { const first = raw.trim().split(";", 1)[0]; const eq = first.indexOf("="); if (eq > 0) jar.set(first.slice(0, eq), first); }
  return [...jar.values()].join("; ");
}
function rscBlob(html: string): string {
  const chunks = html.match(/self\.__next_f\.push\(\[1,"((?:[^"\\]|\\.)*)"\]\)/g) ?? [];
  const out: string[] = [];
  for (const chunk of chunks) { const match = chunk.match(/^self\.__next_f\.push\(\[1,"((?:[^"\\]|\\.)*)"\]\)$/); if (!match) continue; try { out.push(JSON.parse(`"${match[1]}"`)); } catch {} }
  return out.join("\n");
}
function extractObjects(blob: string, marker: string): JsonObject[] {
  const out: JsonObject[] = []; let cursor = 0;
  while (true) {
    const at = blob.indexOf(marker, cursor); if (at < 0) break; const start = at + marker.length; if (blob[start] !== "{") { cursor = start; continue; }
    let depth = 0, end = -1, inString = false, escaped = false;
    for (let i = start; i < blob.length && i < start + 20000; i++) { const ch = blob[i]; if (escaped) { escaped = false; continue; } if (ch === "\\" && inString) { escaped = true; continue; } if (ch === '"') { inString = !inString; continue; } if (inString) continue; if (ch === "{") depth++; else if (ch === "}") { depth--; if (depth === 0) { end = i; break; } } }
    if (end < 0) break; try { out.push(JSON.parse(blob.slice(start, end + 1)) as JsonObject); } catch {} cursor = end + 1;
  }
  return out;
}
async function searchOfficialUr(companyName: string, timeoutMs: number): Promise<UrResult> {
  ...
  try {
    ...
    cookie = appendSetCookies(cookie, searchRes.headers);
    ...
    if (!companies.length && !publications.length) return { ok: true, data: { available: false, years: [], source: "Unternehmensregister (DE)", note: "Nessuna pubblicazione contabile trovata." } };
    const best = companies.sort((a, b) => similarity(b.name ?? "", companyName) - similarity(a.name ?? "", companyName))[0];
    const companyKey = norm(best?.name ?? companyName);
    const sameCompany = publications.filter((p) => { const key = norm(p.companyNameAtTimeOfPublication ?? ""); return !key || key === companyKey || key.startsWith(companyKey) || companyKey.startsWith(key); });
    sameCompany.sort((a, b) => (b.sourceDate ?? "").localeCompare(a.sourceDate ?? ""));
    const chosen = sameCompany.find((p) => /abschl|bilanz|finanzbericht|annual/i.test(p.title ?? "")) ?? sameCompany[0] ?? publications[0];
    if (!chosen) return { ok: true, data: { available: false, years: [], source: "Unternehmensregister (DE)", note: "Nessuna pubblicazione contabile trovata." } };
    const payload = chosen.payload ?? chosen.encryptedPayload;
    if (!payload) return { ok: true, data: { available: false, years: [], source: "Unternehmensregister (DE)", note: "Pubblicazione trovata ma documento non accessibile automaticamente." } };
    const param = chosen.payload ? "payload" : "encryptedPayload";
    const publicationUrl = `${UR_BASE}/de/veroeffentlichung?${param}=${encodeURIComponent(payload)}`;
    return { ok: true, data: { available: true, years: [], source: `Unternehmensregister (DE) — ${chosen.title ?? "Jahresabschluss"}`, documentUrl: `/api/company-finder/document?url=${encodeURIComponent(publicationUrl)}`, documentTitle: [chosen.title, chosen.companyNameAtTimeOfPublication, chosen.sourceDate].filter(Boolean).join(" · "), note: "Il documento è individuato sul registro ufficiale; il recupero server-side può essere limitato dalla verifica di sicurezza del portale." } };
  } catch (e) { const err = e as { name?: string; message?: string; cause?: { code?: string } } | undefined; const code = err?.cause?.code; return { ok: false, error: `Unternehmensregister: ${err?.name === "AbortError" ? "timeout" : code === "ENOTFOUND" ? "DNS non risolto" : err?.message ?? "errore"}` }; }
...
export async function searchUrAccounting(companyName: string, timeoutMs = 30000): Promise<UrResult> {
  ...
  if (key) {
    ...
    const fallback = await searchOfficialUr(companyName, timeoutMs);
    if (fallback.data && (structured.error || structured.skipped)) fallback.data = { ...fallback.data, note: `OpenRegister non riuscito (${structured.error ?? structured.skipped}). Fallback attivo su Unternehmensregister. ${fallback.data.note ?? ""}`.trim() };
    return fallback;
  }
  return searchOfficialUr(companyName, timeoutMs);
}
