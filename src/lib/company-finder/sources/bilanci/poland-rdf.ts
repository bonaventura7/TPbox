...
function createCookieJar(): CookieJar { return { values: new Map() }; }

function setCookies(jar: CookieJar, headers: Headers): void {
  const getter = (headers as Headers & { getSetCookie?: () => string[] }).getSetCookie;
  const values = typeof getter === "function" ? getter.call(headers) : (headers.get("set-cookie") ?? "") ? (headers.get("set-cookie") ?? "").split(/,(?=[^;=]+=)/) : [];
...
function cookieHeader(jar: CookieJar): string { return [...jar.values].map(([name, value]) => `${name}=${value}`).join("; "); }
function normalizeKrs(value: string): string { return value.replace(/\D/g, "").padStart(10, "0"); }

function warsawParts(date: Date): { year: string; month: string; day: string; hour: string; minute: string; second: string } {
  const parts = new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Warsaw", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit", hourCycle: "h23" }).formatToParts(date);
...
async function bootstrapSession(jar: CookieJar, signal: AbortSignal): Promise<string | undefined> {
  ...
  for (const path of paths) {
    const response = await fetch(`${RDF_ORIGIN}${path}`, { headers: { "User-Agent": UA, Accept: "text/html,application/xhtml+xml", "Accept-Language": "pl-PL,pl;q=0.9,en;q=0.8" }, signal, cache: "no-store" });
    const body = await response.clone().text();
    setCookies(jar, response.headers);
    if (!response.ok) { lastError = new Error(`RDF session HTTP ${response.status}`); continue; }
    if (isRdfWafChallenge(body) && !jar.values.has("XSRF-TOKEN")) { lastError = new Error("RDF challenge: public viewer returned a WAF/Incapsula challenge"); continue; }
...
function requestHeaders(jar: CookieJar, xsrf?: string): Record<string, string> {
  const headers: Record<string, string> = { "User-Agent": UA, "Accept-Language": "pl-PL,pl;q=0.9,en;q=0.8", Origin: RDF_ORIGIN, Referer: `${RDF_ORIGIN}/wyszukaj-podmiot` };
...
function firstNumber(record: Record<string, unknown>, keys: string[]): number | undefined {
  const value = firstString(record, keys); if (!value) return undefined;
  const match = value.match(/\b(19|20)\d{2}\b/); if (!match) return undefined;
  const year = Number(match[0]); return year >= 1900 && year <= 2100 ? year : undefined;
}
function documentFormat(record: Record<string, unknown>): PolishAnnualDocument["format"] {
  const text = JSON.stringify(record).toLowerCase();
  if (text.includes("pdf")) return "pdf"; if (text.includes("xhtml") || text.includes("xbrl")) return "xbrl"; if (text.includes("xml")) return "xml"; if (text.includes("zip")) return "zip"; if (text.includes("html")) return "html"; return "unknown";
}
function looksLikeAnnualReport(record: Record<string, unknown>): boolean {
  const text = JSON.stringify(record).normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
...
function collectDocumentRecords(value: unknown, out: Record<string, unknown>[] = []): Record<string, unknown>[] {
  if (Array.isArray(value)) { for (const item of value) collectDocumentRecords(item, out); return out; }
  const record = asObject(value); if (!record) return out;
  if (looksLikeAnnualReport(record)) out.push(record);
  for (const child of Object.values(record)) collectDocumentRecords(child, out);
  return out;
}
function documentId(record: Record<string, unknown>): string | undefined { return firstString(record, ["idDokumentu", "identyfikatorDokumentu", "documentId", "identyfikator", "id"]); }
function documentTitle(record: Record<string, unknown>): string { return firstString(record, ["nazwaDokumentu", "rodzajDokumentu", "typDokumentu", "nazwa", "opis"]) ?? "Roczne sprawozdanie finansowe"; }

export function extractPolishAnnualDocuments(payload: unknown, requestedYear?: number): PolishAnnualDocument[] {
  const records = collectDocumentRecords(payload); const seen = new Set<string>(); const documents: PolishAnnualDocument[] = [];
  for (const record of records) {
    const id = documentId(record); if (!id || seen.has(id)) continue;
...
async function fetchJsonWithSession(krs: string, jar: CookieJar, signal: AbortSignal): Promise<unknown> {
  let xsrf = await bootstrapSession(jar, signal); const encryptedKrs = encryptPolishKrs(krs);
  const request = () => fetch(`${RDF_ORIGIN}${SEARCH_PATH}`, { method: "POST", headers: { ...requestHeaders(jar, xsrf), "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ nrKRS: encryptedKrs, metadaneStronicowania: { numerStrony: 0, rozmiarStrony: 100, metadaneSortowania: [{ atrybut: "id", kierunek: "MALEJACO" }] } }), signal, cache: "no-store" });
  let response = await request();
  if (response.status === 401 || response.status === 403) { jar.values.clear(); xsrf = await bootstrapSession(jar, signal); response = await request(); }
...
export async function searchPolishAnnualReports(krsNumber: string, requestedYear?: number, timeoutMs = TIMEOUT_MS): Promise<PolishAnnualSearchResult> {
  const krs = normalizeKrs(krsNumber); if (!/^\d{10}$/.test(krs)) return { ok: false, documents: [], error: "KRS non valido" };
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const payload = await fetchJsonWithSession(krs, createCookieJar(), controller.signal); const documents = extractPolishAnnualDocuments(payload, requestedYear);
    return documents.length ? { ok: true, documents } : { ok: false, documents: [], error: requestedYear ? `bilancio ${requestedYear} non trovato nel RDF KRS` : "nessun bilancio individuato nel RDF KRS" };
  } catch (error) {
...
async function fetchDocumentContent(krsNumber: string, documentIdValue: string, timeoutMs: number): Promise<PolishFinancialDocument> {
  ...
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), timeoutMs); const jar = createCookieJar();
  try {
    const xsrf = await bootstrapSession(jar, controller.signal);
    const response = await fetch(`${RDF_ORIGIN}${CONTENT_PATH}/${encodeURIComponent(documentIdValue)}/tresc?bezPodpisu=false`, { headers: { ...requestHeaders(jar, xsrf), Accept: "application/pdf,application/octet-stream,application/xml,text/xml,text/html,*/*" }, signal: controller.signal, cache: "no-store" });
    setCookies(jar, response.headers); if (!response.ok) throw new Error(`RDF documento HTTP ${response.status}`);
    const arrayBuffer = await response.arrayBuffer(); if (arrayBuffer.byteLength > MAX_BYTES) throw new Error("documento troppo grande");
    const bytes = new Uint8Array(arrayBuffer); const contentType = (response.headers.get("content-type") ?? "application/octet-stream").toLowerCase();
...
export async function fetchPolishAnnualReport(krsNumber: string, year: number, timeoutMs = TIMEOUT_MS): Promise<PolishFinancialDocument & { document?: PolishAnnualDocument }> {
  ...
  const primaryBudget = Math.min(4_500, Math.max(1_000, Math.floor(timeoutMs * 0.6)));
  let primaryError = `bilancio ${year} non disponibile`;

  const found = await searchPolishAnnualReports(krs, year, Math.min(primaryBudget, remaining()));
