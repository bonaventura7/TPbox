...
async function bootstrapSession(jar: CookieJar, signal: AbortSignal): Promise<string | undefined> {
  ...
  for (const path of paths) {
    const response = await fetch(`${RDF_ORIGIN}${path}`, {
      headers: { "User-Agent": UA, Accept: "text/html,application/xhtml+xml", "Accept-Language": "pl-PL,pl;q=0.9,en;q=0.8" },
...
function requestHeaders(jar: CookieJar, xsrf?: string): Record<string, string> {
  const headers: Record<string, string> = {
    "User-Agent": UA, "Accept-Language": "pl-PL,pl;q=0.9,en;q=0.8", Origin: RDF_ORIGIN, Referer: `${RDF_ORIGIN}/wyszukaj-podmiot`,
  };
...
function firstNumber(record: Record<string, unknown>, keys: string[]): number | undefined {
  const value = firstString(record, keys); if (!value) return undefined; const match = value.match(/\b(19|20)\d{2}\b/); if (!match) return undefined;
...
export function extractPolishAnnualDocuments(payload: unknown, requestedYear?: number): PolishAnnualDocument[] {
  ...
  for (const record of records) {
    ...
    documents.push({ id, year, title: documentTitle(record), format: documentFormat(record) === "unknown" ? "pdf" : documentFormat(record) }); seen.add(id);
  }
...
async function fetchJsonWithSession(krs: string, jar: CookieJar, signal: AbortSignal): Promise<unknown> {
  ...
  const request = () => fetch(`${RDF_ORIGIN}${SEARCH_PATH}`, {
    method: "POST", headers: { ...requestHeaders(jar, xsrf), "Content-Type": "application/json", Accept: "application/json" },
    body: JSON.stringify({ nrKRS: encryptedKrs, metadaneStronicowania: { numerStrony: 0, rozmiarStrony: 100, metadaneSortowania: [{ atrybut: "id", kierunek: "MALEJACO" }] } }), signal, cache: "no-store",
  });
...
async function fetchDocumentContent(krsNumber: string, documentIdValue: string, timeoutMs: number): Promise<PolishFinancialDocument> {
  const krs = normalizeKrs(krsNumber); if (!/^\d{10}$/.test(krs) || !documentIdValue.trim()) return { ok: false, bytes: new Uint8Array(), contentType: "", error: "parametri documento non validi" };
...
export async function fetchPolishFinancialDocument(krsNumber: string, documentIdValue: string, timeoutMs = TIMEOUT_MS): Promise<PolishFinancialDocument> { return fetchDocumentContent(krsNumber, documentIdValue, timeoutMs); }
...
export async function fetchPolishAnnualReport(krsNumber: string, year: number, timeoutMs = TIMEOUT_MS): Promise<PolishFinancialDocument & { document?: PolishAnnualDocument }> {
  ...
  if (!found.ok || !found.documents[0]) return { ok: false, bytes: new Uint8Array(), contentType: "", error: found.error ?? `bilancio ${year} non disponibile` };
  const document = found.documents[0]; const result = await fetchDocumentContent(krsNumber, document.id, timeoutMs); return { ...result, document };
}
