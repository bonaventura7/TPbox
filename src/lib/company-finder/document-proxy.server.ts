...
function cookieHeader(j: CookieJar): string { return [...j.values].map(([key, value]) => `${key}=${value}`).join("; "); }
function isPdf(bytes: ArrayBuffer): boolean { return new TextDecoder("latin1").decode(new Uint8Array(bytes).slice(0, 8)).startsWith("%PDF-"); }
...
async function fetchRaw(target: URL, signal: AbortSignal, accept: string, cookies: CookieJar, ref?: URL): Promise<Fetched> {
  let current = target;
  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    const headers: Record<string, string> = { "User-Agent": UA, Accept: accept, "Accept-Language": "de-DE,de;q=0.9,en;q=0.8" };
    const cookie = cookieHeader(cookies);
    if (cookie) headers.Cookie = cookie;
    if (ref) headers.Referer = ref.toString();
    const response = await fetch(current.toString(), { headers, signal, redirect: "manual", cache: "no-store" });
    ...
    return { bytes, contentType: (response.headers.get("content-type") ?? "").toLowerCase(), finalUrl: current };
  }
  throw new Error("troppi redirect");
}
async function bootstrap(cookies: CookieJar, signal: AbortSignal): Promise<void> {
  try { await fetchRaw(new URL("https://www.unternehmensregister.de/de/suche"), signal, "text/html,application/xhtml+xml", cookies); } catch {}
}
function serve(doc: Fetched, download: boolean): Response {
  const pdf = isPdf(doc.bytes) || doc.contentType.includes("pdf");
  return new Response(doc.bytes, { headers: { "Content-Type": pdf ? "application/pdf" : doc.contentType || "text/html; charset=utf-8", "Content-Disposition": `${download ? "attachment" : "inline"}; filename="${pdf ? "bilancio.pdf" : "bilancio.html"}"`, "Cache-Control": "no-store", "X-Content-Type-Options": "nosniff" } });
}
function unwrap(requestUrl: string, target: string, accept: string): { target: string; accept: string } {
  let current = target; let nextAccept = accept;
  for (let i = 0; i < 3; i++) {
    if (!current.startsWith("/api/company-finder/document?")) break;
    const url = new URL(current, requestUrl); const nested = url.searchParams.get("url"); if (!nested) break;
    current = nested; nextAccept = url.searchParams.get("accept") || nextAccept;
  }
  return { target: current, accept: nextAccept };
}
export async function handleDocumentRequest(request: Request): Promise<Response> {
  const params = new URL(request.url).searchParams; const raw = params.get("url"); if (!raw) return fail("url mancante", 400);
  const unwrapped = unwrap(request.url, raw, params.get("accept") || "*/*"); let source: URL;
  try { source = new URL(unwrapped.target); } catch { return fail("url non valida", 400); }
  const httpOnly = HTTP_ONLY_HOSTS.has(source.hostname.toLowerCase());
  if (source.protocol !== "https:" && !(source.protocol === "http:" && httpOnly)) return fail("sono ammesse solo url https", 400);
  if (!isAllowedDocumentHost(source)) return fail("dominio non autorizzato", 403);
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), TIMEOUT_MS); const cookies = jar();
  try {
    if (source.hostname.endsWith("unternehmensregister.de")) await bootstrap(cookies, controller.signal);
    const first = await fetchRaw(source, controller.signal, unwrapped.accept, cookies); const download = params.get("download") === "1";
    if (!isHtml(first.contentType, first.bytes)) {
      if (isPdf(first.bytes) || /pdf|octet-stream|zip|xml/i.test(first.contentType)) return serve(first, download);
    ...
    for (const link of linksFrom(text)) {
      const next = allowed(link, first.finalUrl); if (!next) continue;
      try {
        const document = await fetchRaw(next, controller.signal, "application/pdf,application/octet-stream,*/*", cookies, first.finalUrl);
        if (isPdf(document.bytes) || document.contentType.includes("pdf")) return serve(document, download);
      } catch {}
    }
    return serve(first, download);
  } catch (error) {
    const err = error as { name?: string; message?: string };
    return fail(`impossibile recuperare il documento: ${err?.name === "AbortError" ? "timeout" : err?.message ?? "errore di rete"}`, 502);
