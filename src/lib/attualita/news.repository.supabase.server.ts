...
import type { NewsFeedResult, NewsFilters, NewsItem, ServiceHealth } from "../domain/types";
...
export async function listNewsFeed(filters: NewsFilters): Promise<NewsFeedResult> {
  ...
  const lastPipelineRunAt =
    items.reduce<string | null>((latest, item) => {
      if (!latest || item.lastVerifiedAt > latest) return item.lastVerifiedAt;
      return latest;
    }, null);
