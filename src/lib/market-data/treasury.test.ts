...
import { TREASURY_CACHE_TTL_MS } from "./connectors.server";
...
describe("Treasury daily par yield XML", () => {
  ...
  it("parses one XML document into observations for every CMT tenor", () => {
    const parsed = parseTreasuryXml(xml);

    expect(parsed.get("BC_3MONTH")).toEqual([{ period: "2026-09-02", value: 3.92 }]);
    expect(parsed.get("BC_5YEAR")).toEqual([{ period: "2026-09-02", value: 4.54 }]);
    expect(parsed.get("BC_10YEAR")).toEqual([{ period: "2026-09-02", value: 4.79 }]);
    expect(parsed.size).toBe(8);
  });

  it("uses a 15-minute Treasury cache TTL", () => {
    expect(TREASURY_CACHE_TTL_MS).toBe(15 * 60 * 1000);
  });
